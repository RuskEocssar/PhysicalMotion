...
| `entity.hit.enable` | 弾がヒットするかどうか(ヒットする) | 一部の非モブのエンティティはヒット判定をとらないようになっているが、強制的にヒット判定を持つようにするタグ。
| `entity.hit.disable` | 弾がヒットするかどうか(ヒットしない) | ヒット判定を持つエンティティにたいして、ヒット判定を持たないようにするタグ。