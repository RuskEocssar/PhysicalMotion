...
## 入力の取得
    data modify storage pm_motion:zz/core in set from storage pm_motion: in_default
    data modify storage pm_motion:zz/core in merge from storage pm_motion: in

## データを設定
    # パラメータの保存
    data modify entity @s data.pmm set from storage pm_motion:zz/core in
    # 初速
    execute store result score #pmmH.1 pmmS.x.1 run data get storage pm_motion:zz/core in.velocity 100
    ...
    # その他
    execute store result score @s pmmS.time run data get storage pm_motion:zz/core in.max_time
    execute store result score @s pmmS.distance run data get storage pm_motion:zz/core in.max_distance 100
    execute store result score @s pmmS.count run data get storage pm_motion:zz/core in.max_reflect
