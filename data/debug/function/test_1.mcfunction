execute anchored eyes positioned ^ ^ ^ run summon marker ~ ~ ~ {Tags:[test,ray]}

# execute anchored eyes positioned ^ ^ ^1 run summon item_display ~ ~ ~ {Tags:[test],Passengers:[{id:"minecraft:mannequin",DeathTime:20,Tags:[pmmT.through],profile:{name:"Rusk_Eocssar"},attributes:[{id:"scale",base:0.5f}]}]}
# execute anchored eyes positioned ^ ^ ^1 run summon item_display ~ ~ ~ {Tags:[test],Passengers:[{id:"minecraft:item_display",item:{id:"stone"}}]}
# execute anchored eyes positioned ^ ^ ^1 run summon minecraft:mannequin ~ ~ ~ {Tags:[test],profile:{name:"Rusk_Eocssar"},attributes:[{id:"scale",base:0.4f}]}
data modify storage pm_motion: in set value {size:6,max_reflect:10,velocity:10,events:{trail:"particle dust{color:[1,0,0],scale:1} ~ ~ ~ 0 0 0 0.001 1 force",last:"function pm_motion:kill_all",hit_entity:"execute as @e[tag=pmmT.hitted] run damage @s 5 minecraft:player_attack"},max_distance:100,c_drag:0,max_time:200,gravity:0}
execute as @e[tag=test] positioned ~ ~-0.5 ~ run function pm_motion:motion

tag @e[tag=test] remove test