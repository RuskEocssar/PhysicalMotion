...
## 反射
    # 反射方向を取得
    execute facing entity @e[tag=pmmT.hitted,limit=1,sort=nearest] eyes facing ^ ^ ^-1 positioned 0. 0. 0. run tp 4fe002bb-0-0-0-1 ^ ^ ^1
    execute store result score #pmmH.2 pmmS.1 store result score #pmmH.2 pmmS.x.2 run data get entity 4fe002bb-0-0-0-1 Pos[0] 100
    execute store result score #pmmH.2 pmmS.y.1 store result score #pmmH.2 pmmS.y.2 run data get entity 4fe002bb-0-0-0-1 Pos[1] 100
    execute store result score #pmmH.2 pmmS.z.1 store result score #pmmH.2 pmmS.z.2 run data get entity 4fe002bb-0-0-0-1 Pos[2] 100
    # 法線の変化量を計算
